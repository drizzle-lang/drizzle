...
module Drizzle
  ...
  class Parser
    ...
    def initialise_parsers
      ...
      add_infix LEFT_BRACKET, parse_index_expression
    end
    ...
    # Parse an index expression found at the current token
    # `expression[expression]`
    def parse_index_expression(left : AST::Expression) : AST::Expression
      token = @current
      self.next_token
      index = parse_expression Precedence::LOWEST
      if !eat? TokenType::RIGHT_BRACKET
        index = AST::Identifier.new(@peek, "error").as AST::Expression
      end
      return AST::IndexExpression.new token, left, index.not_nil!
    end
